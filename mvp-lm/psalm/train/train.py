...
from psalm.model.language_model.openseed_phi import PSALM
# from  psalm.model.language_model.llava_phi import PSALM
...
class DataArguments:
    ...
    ref_caption_path: Optional[str] = "/path/to/captions_train.json"
    ...
    o365_image_root: str = '/path/to/o365/image_root'
    o365_json_path: str = '/path/to/o365/instance_train.json'
    grit_h5_path: str = '/path/to/grit/dataset.h5'
    mmconv_path: str = '/path/to/llava_1_5'
    cc3m_data_path: str = '/path/to/CC3M/chat.json'
    cc3m_image_folder: str = '/path/to/CC3M/images'
    mixed_data_path: str = '/path/to/MIXED/annos.json'
    mixed_image_folder: str = '/path/to/MIXED/images'
    flickr_data_path: str = '/path/to/flickr/annos.json'
    flickr_image_folder: str = '/path/to/flickr/images'
    reason_image_folder: str = '/path/to/reason_seg/images'
    data_ratio: str = '1||1||1||1||1'
...
def make_unify_datamodule(tokenizer, data_args, training_args):
    ...
    o365_dataset = O365_detection_dataset(image_root=data_args.o365_image_root, json_path=data_args.o365_json_path,
                                          tokenizer=tokenizer, data_args=data_args)
    
    mixed_grounding_dataset = MIXED_grounding_dataset(data_path=data_args.mixed_data_path, image_folder=data_args.mixed_image_folder,  tokenizer=tokenizer,
                                                      data_args=data_args)

    flickr30k_grounding_dataset = MIXED_grounding_dataset(data_path=data_args.flickr_data_path, image_folder=data_args.flickr_image_folder,  tokenizer=tokenizer,
                                                      data_args=data_args)
    ...
    # # cc3m_dataset = CC3M_Dataset(data_path=data_args.cc3m_data_path, image_folder=data_args.cc3m_image_folder,  tokenizer=tokenizer,
    # #                                                          data_args=data_args)
    
    # # reason_dataset = Reason_dataset(image_folder=data_args.reason_image_folder, tokenizer=tokenizer,
    # #                                                   data_args=data_args)
    ...
    # datasets = [panoptic_coco_dataset]*data_ratio[0]

    # datasets = [panoptic_coco_dataset]*data_ratio[0] + [o365_dataset]*data_ratio[4] 
    # datasets = [panoptic_coco_dataset]*data_ratio[0] + [refcoco_dataset] * data_ratio[1]

    # datasets = [panoptic_coco_dataset]*data_ratio[0] + [refcoco_dataset] * data_ratio[1] \
    #              + [o365_dataset]*data_ratio[4] # + [mm_conv_dataset]*data_ratio[3]
    
    datasets = [panoptic_coco_dataset]*data_ratio[0] + [refcoco_dataset] * data_ratio[1] \
         + [o365_dataset]*data_ratio[4] \
        + [mixed_grounding_dataset]*data_ratio[5] + [flickr30k_grounding_dataset]*data_ratio[6] \
    #     + [mm_conv_dataset]*data_ratio[3] # + [cc3m_dataset]*data_ratio[7] 
    ...
    if training_args.lora_enable:
        ...
        for n, p in model.named_parameters():
            if any(
                [
                    x in n
                    for x in ["seg_query", "embed_tokens", "lm_head", "region_projector", 
                              "pixel_decoder", "predictor", 
                              "seg_query_projector", "class_name_projector", "SEG_token_projector",
                              "expanded_seg_query_project",
                              "region_sampler", 'pe_layer',]
                ]):
