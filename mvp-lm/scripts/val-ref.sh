# folder=openseed_TwoStage_percept_ref_100q_hyper
# folder=openseed_TwoStage_coco_o365_100q_hyper
#!/bin/bash
...
for folder in "${folder_array[@]}"; do
    ...
    script_path=${project_root_path}'/mvp-lm/psalm/eval/referring_segmentation.py';
    task_type='referringseg';
    ...
    PYTHONPATH=${project_root_path}/mvp-lm python3 -m torch.distributed.run --master_port ${PORT} --nproc_per_node=${gpu_count} ${script_path} \
        ...
        --json_path /ABSOLUTE_PATH_TO_IMAGE_DATASET/refer/refcoco/val_psalm.json \
        --image_folder /ABSOLUTE_PATH_TO_IMAGE_DATASET/coco/train2014 \
        --lora_enable False \
