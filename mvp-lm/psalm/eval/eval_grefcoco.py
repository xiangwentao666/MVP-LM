...
class DataArguments:
    ...
    ref_caption_path: Optional[str] = field(default=None)
    # 添加分布式相关参数
    local_rank: int = field(default=0, metadata={"help": "Local rank for distributed training."})
    distributed: bool = field(default=False, metadata={"help": "Whether to use distributed training."})

    visualize: bool = False


def init_distributed_mode(data_args):
    if 'WORLD_SIZE' in os.environ and int(os.environ['WORLD_SIZE']) > 1:
        data_args.distributed = True
    else:
        data_args.distributed = False

    if data_args.distributed:
        # 初始化分布式环境
        dist.init_process_group(backend="nccl", init_method='env://')
        data_args.local_rank = int(os.environ.get('LOCAL_RANK', 0))
        torch.cuda.set_device(data_args.local_rank)
        data_args.world_size = dist.get_world_size()
        print(f'I am rank {dist.get_rank()} in this world of size {data_args.world_size}!')
    else:
        data_args.local_rank = 0
...
class gRefcoco_Dataset(RefCOCO_dataset):
    def __getitem__(self, idx):
        ...
        prefix_inst = 'This is an image <image>, Please do Referring Segmentation according to the following instruction:'
        instruction = ''
...
def evaluation(data_args, thr=0.6):
    disable_torch_init()
    init_distributed_mode(data_args)  # 初始化分布式环境
    ...
    tokenizer, model, image_processor, context_len = load_pretrained_model(
        model_path, None, model_name, model_args=data_args, mask_config=data_args.mask_config, device='cuda'
    )
    ...
    data_args.output_dir = data_args.output_dir + "_" + os.path.basename(data_args.json_path).split('.')[0]
    ...
    if data_args.distributed:
        val_sampler = torch.utils.data.distributed.DistributedSampler(eval_dataset, shuffle=False)
    else:
        val_sampler = None
    ...
    try:
        ...
        print('dataset have been registered')
    MetadataCatalog.get('refcoco_dataset').set(stuff_classes=['object'], )
    metadata = MetadataCatalog.get('refcoco_dataset')

    gt_json_path = data_args.json_path
    with open(gt_json_path) as f:
        gt_data = json.load(f)

    device = torch.device(f"cuda:{data_args.local_rank}" if torch.cuda.is_available() else "cpu")
    ...
    if data_args.distributed:
        model = torch.nn.parallel.DistributedDataParallel(
            model,
            device_ids=[data_args.local_rank],
            output_device=data_args.local_rank,
            find_unused_parameters=True  # 根据模型情况设置
        )
    else:
    ...
    with torch.no_grad():
        for idx, inputs in tqdm(enumerate(eval_dataloader), total=len(eval_dataloader)):
            if data_args.distributed:
                val_sampler.set_epoch(idx)  # 为 DistributedSampler 设置 epoch

            gt = gt_data[idx]['anns']
            gt = inputs['seg_info'][0]['annotations']
            h, w = gt_data[idx]['image_info']['height'], gt_data[idx]['image_info']['width']
            h, w = inputs['seg_info'][0]['height'], inputs['seg_info'][0]['width']
            # 生成 ground truth mask
            masks = []
            ...
            outputs, seg_query, class_name_embedding = model.module.eval_seg(
            ...
            if data_args.visualize:
                if not os.path.exists(os.path.join(data_args.output_dir, 'vis')):
                ...
                save_path = os.path.join(data_args.output_dir, 'vis', fn)
                item_list = os.listdir(os.path.join(data_args.output_dir, 'vis'))
                item_list = [item for item in item_list if item.startswith(fn.replace(".jpg", ""))]
                save_path = save_path.replace(".jpg", "-{}.jpg".format(len(item_list)))
                print("save to:\t", save_path)
                demo.save(save_path)
                prompt = "".join([tokenizer.decode(item) for item in inputs['input_ids'][-1] if item >= 0])
                refer = tokenizer.decode(inputs['token_refer_id'][0])
                print(prompt, "\n\n", refer)
                with open(os.path.join(data_args.output_dir, 'vis', os.path.basename(save_path).replace('.jpg', "-refer.txt")), 'w') as f:
                    f.write(refer)
                if idx > 40: break
    ...
    if data_args.distributed:
        ...
        intersection_meter.sum = intersection_meter.sum
        union_meter.sum = union_meter.sum
        acc_iou_meter.avg = acc_iou_meter.avg
