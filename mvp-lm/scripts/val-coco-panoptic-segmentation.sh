...
script_path=${project_root_path}'/mvp-lm/psalm/eval/panoptic_segmentation.py';
task_type='panoptic_segmentation';
json_path_coco="" # path to coco
#####################################################################
...
PYTHONPATH=${project_root_path}/mvp-lm python3 -m torch.distributed.run --master_port ${PORT} --nproc_per_node=${gpu_count} ${script_path} \
    --json_path ${json_path_coco} \
    --vision_tower ${vision_tower_path} \
    --pretrain_mm_mlp_adapter ${pretrain_mm_mlp_adapter} \
    --lora_r 8 \
    --lora_enable False \
    --mask_config ${mask_config} \
    --output_dir ${output_dir}-coco_panoptic_segmentation \
