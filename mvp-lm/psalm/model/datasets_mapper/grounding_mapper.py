...
class GroundingMapper:
    ...
    def preprocess(self, dataset_dict):
        """
        Args:
        ...
        if 'http://' not in dataset_dict['file_name'] or 'https://' not in dataset_dict['file_name']:
            ...
            while stat != 200:
                try:
                    response = requests.get(dataset_dict['file_name'])
                    # 将内容读取到内存中的BytesIO对象
        ...
        if "bbox" in dataset_dict:
            ...
            # obj = {'bbox': np.array([0.0, 0.0, 1.0, 1.0]).reshape(-1,4)}
            # obj['bbox_mode']=BoxMode.XYXY_ABS
            # obj['category_id'] = 0
            # annos.append(utils.transform_instance_annotations(obj, transforms, image_shape))
            img_wh = np.array([origin_image_shape[1], origin_image_shape[0], origin_image_shape[1], origin_image_shape[0]])
            for ii, bbox in enumerate(dataset_dict["bbox"]):
                bbox = bbox * img_wh
                obj = {'bbox': bbox[None]}
                obj['bbox_mode']=BoxMode.XYXY_ABS
                obj['category_id'] = ii
            ...
            instances.gt_classes = instances.gt_classes.to(torch.int64)
